...
rf:
	...
	$(call colorecho, "\nPlease wait few seconds to contianers fully intitialise")
...
PHONY += ssh-client
ssh-client:		## SSH to Client JS container (React App)
ssh-client:
...
PHONY += tf
tf:			## Run Javascript lint tests
tf:
	$(call colorecho, "\nLint testing JS code on client:\n")
	@cd ./client; npm run lint-js

PHONY += lcsss
lcsss:			## Run Javascript lint tests
lcsss:
	$(call colorecho, "\nLint testing JS styled code on client:\n")
	@cd ./client; npm run lint:css

PHONY += fcpd
fcpd:			## Copy Paste detection of JS code
fcpd:
	$(call colorecho, "\nChecking Copy Paste detection of JS code :\n")
	@cd ./client; npm run jscpd

PHONY += lcss
lcss:			## Run SASS lint tests
lcss:
	$(call colorecho, "\nLint testing JS code on client:\n")
...
PHONY += logs-client
logs-client:		## View Logs from client JS Contianer (React App)
logs-client:
